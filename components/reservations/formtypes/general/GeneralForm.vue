...
import ReservationStep1 from "@/components/reservations/formtypes/general/steps/ReservationStep1.vue";
import ReservationStep2 from '@/components/reservations/formtypes/general/steps/ReservationStep2.vue';
import ReservationStep3 from '@/components/reservations/formtypes/general/steps/ReservationStep3.vue';
...
const reservationType = ref('general')
...
onMounted(() => {
  ...
  if (!hasExistingReservation) {
    ...
    console.log("🧹 Inicializando nueva reservación general - limpiando estado previo");
    resetForm();
  } 
  else {
...
const handleNavigateToStep = (step: number): void => {
  ...
  setTimeout(() => {
    emitStepNavigationEvent(step, "general", currentReservationId.value);
  }, 100);
}
...
const handleStep1Submit = (result: any) => {
  ...
  setTimeout(() => {
    if (currentReservationId.value) {
      emitReservationIdSetEvent(currentReservationId.value, "general");
    }
  }, 100);
}
...
const handleCompleteReservation = async (finalData: any) => {
  try {
    ...
    // Navegar a la página de vinculación con el ID de la reservación
    if (reservationId) {
      ...
      // Comentado: navegación anterior a página de éxito
      // await router.push(`/reservations/general/success?reservationId=${reservationId}`)
  
      await router.push(`/vinculation?reservationId=${reservationId}`)
    } else {
      await router.push('/reservations/general/success')
    }
