...
import ReservationSuccess from '@/components/reservations/formtypes/general/ReservationSuccess.vue'
...
const { store, clearPersistence } = useReservationGeneral()
...
const handleDownload = (reservationData) => {
  ...
  downloadPDF(pdfContent, `reservacion-general-${reservationData?.id || 'confirmacion'}.pdf`)
}
...
const generatePDFContent = (data) => {
  return {
    title: 'Confirmación de Reservación General',
...
onMounted(() => {
  ...
  showSuccess(
    ...
    'Tu reservación general ha sido procesada exitosamente'
  )
})
